...
function CalendarGrid({gridTemp, events, handleView, handleDragStart, handleDragEnd, handleDragAnimation, handleLeave, week, year, deletingEvent}) {
  ...
  return (
    <>
        <div className='w-full h-full grid grid-cols-5 grid-rows-12 border-t-2 border-t-stone-400 border-x-stone-400 border-x-2'>
          {gridTemp.map((events, index)=>{
            return(
              <div key={index} className='flex w-full h-full border-y border-y-stone-800 border-x border-x-stone-600 justify-center items-center'></div>
            )
          })}
          <div id="division" onMouseDown={handleDragStart} onMouseUp={handleDragEnd} onMouseLeave={handleLeave} onMouseMove={handleDragAnimation} className="absolute w-[calc(100svw_-_6.5rem)] h-[calc(79svh_-_1.4rem)] mr-10">
            <div id="draggable" className=""></div>
            <div id="indicator" className=""></div>
            {events.filter(e => e.week === week && e.year === year).map((e, index)=>{
                return(
                    <div onClick={()=>handleView(e.name, e.start, e.end, e.day, e.description, e.date, e.year, e.week, e.colorOne, e.colorTwo)} key={index} className={`absolute h-[calc(8.36%_*_${e.end-e.start})] left-[calc(20%_*_${e.day-1})] top-[calc(8.33%_*_${e.start-8})] w-1/5 bg-gradient-to-b from-[${e.colorOne}] to-[${e.colorTwo}] bg-repeat rounded-xl border-2 border-stone-300 text-white ${deletingEvent?"hover:border-rose-500 hover:border-4 cursor-not-allowed":"hover:border-4 cursor-pointer"}`}>
